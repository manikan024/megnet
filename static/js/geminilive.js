...
function parseResponseMessages(data) {
  ...
  try {
    ...
    if (data?.error) {
      ...
      responses.push({
        type: MultimodalLiveResponseType.ERROR,
...
class GeminiLiveAPI {
  constructor(token, model) {
    ...
    // Server VAD + audioStreamEnd when user taps mic off (no manual activityStart)
    this.automaticActivityDetection = { disabled: false };
  ...
  sendActivityStart() {
    this.sendMessage({ realtimeInput: { activityStart: {} } });
  }

  sendActivityEnd() {
    this.sendMessage({ realtimeInput: { activityEnd: {} } });
  }
